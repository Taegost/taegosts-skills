...
| [claude-code-plugin-script-path-resolution.md](./claude-code-plugin-script-path-resolution.md) | Marketplace-installed skills report the plugin's shared scripts as "not found" when invoked (Issue #115): commands such as `scripts/context-gather.sh` fail with a missing-file error, while the same... |
