...
- Research: Plugin Cache and Reload Behavior (removed from `docs/plans/` in ad3b560; recoverable from git history) — carried a dated correction answering the cache-path question the research never posed; the canonical rule now lives in [Script Extraction Standards](../../standards/script-extraction-standards.md)
