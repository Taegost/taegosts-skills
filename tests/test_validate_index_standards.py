...
GENERATOR = Path(__file__).resolve().parent.parent / "scripts" / "index-scripts.py"
...
# ── Resolution note compatibility (Issue #115) ──

class TestResolutionNoteCompatibility:
    """An INDEX.md carrying the generator's script path resolution note must
    pass the validator cleanly (Issue #115 remediation: the note introduced
    by the generators names ${CLAUDE_PLUGIN_ROOT} paths and must not trip
    R7/R8)."""

    def test_inline_note_bearing_index_passes(self, tmp_path):
        """Hand-built INDEX.md containing the resolution note passes R7/R8."""
        content = (
            "---\ntitle: Test index\ndescription: Test index\nstatus: active\n"
            'version: "1.0"\ncreated: 2026-07-07\nlast-updated: 2026-07-07\n'
            'owner: test-owner\ndependencies: []\ntags: [index]\n---\n\n'
            ...
            "${CLAUDE_PLUGIN_ROOT}/<repo-relative-path>; on other platforms, "
            "resolve from the loaded skill directory or the taegosts-skills "
            "checkout.\n\n"
            "| Link | Description |\n"
            "|------|-------------|\n"
            "| [readme](readme.md) | Project readme |\n"
        )
        result, rc = run_validator(content, "INDEX.md", tmp_path)
        assert rc == 0, f"Validator should pass, got rc={rc}: {result}"
        assert result.get("violations") == [], (
            f"Expected no violations, got {result.get('violations')}"
        )

    def test_generator_output_with_resolution_note_passes(self, tmp_path):
        """INDEX.md generated by index-scripts.py validates with exit 0 and
        no violations."""
        scripts_dir = tmp_path / "scripts"
        scripts_dir.mkdir()
        (scripts_dir / "tool-alpha.sh").write_text(
            "#!/usr/bin/env bash\n# tool-alpha.sh -- Does thing alpha\n"
        )
        (scripts_dir / "tool-beta.py").write_text(
            '#!/usr/bin/env python3\n"""\ntool-beta.py -- Does thing beta.\n"""\n'
        )

        gen = subprocess.run(
            [sys.executable, str(GENERATOR), "--dir", str(scripts_dir)],
            capture_output=True, text=True, check=False,
        )
        ...
        result, rc = run_validator(content, "INDEX.md", scripts_dir)
        assert rc == 0, f"Validator should pass, got rc={rc}: {result}"
        assert result.get("violations") == [], (
            f"Expected no violations, got {result.get('violations')}"
        )
