...
chmod +x deploy.sh
output=$("$SCRIPT" main 2>&1)
if echo "$output" | grep -q "deploy.sh"; then
...
output=$("$SCRIPT" main 2>&1)
if ! echo "$output" | grep -q "CHANGELOG.md"; then
...
output=$("$SCRIPT" main 2>&1)
if ! echo "$output" | grep -q "tests/test_stuff.py"; then
...
output=$("$SCRIPT" main 2>&1)
if ! echo "$output" | grep -q "test-foo.sh"; then
