...
| [default-branch.sh](default-branch.sh) | Resolve default branch with cascading fallbacks |
| [detect-diff-scope.sh](detect-diff-scope.sh) | Compute diff scope and detect which reviewers apply |
| [extract-ktds.py](extract-ktds.py) | Extract Key Technical Decisions from plan markdown files |
| [git-context.sh](git-context.sh) | Unified git state snapshot as JSON |
...
| [run-id.sh](run-id.sh) | Generate timestamp-hex run ID |
| [run-shellcheck.sh](run-shellcheck.sh) | Run shellcheck on all test scripts |
