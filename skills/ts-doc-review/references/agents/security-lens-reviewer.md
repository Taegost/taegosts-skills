...
**Credentials in non-Secret resources.** If the orchestrator's prompt includes K8s security scan findings (see SKILL.md's "K8s security scan" step), treat `check-credentials-in-configmaps.py`'s output as the deterministic source of truth for which ConfigMaps actually contain credential-shaped values — don't re-derive the pattern match by hand. Your job is the security *implication*: is a flagged credential's exposure a real gap given the plan's context, or already mitigated elsewhere (e.g., about to move to a SealedSecret)? Flag when a credential appears in both a ConfigMap (plaintext) and a SealedSecret (encrypted) — the ConfigMap copy defeats the purpose of encryption. If no scan findings were passed (document had no K8s relevance, or the scan errored), fall back to reading Deployment/ConfigMap specs directly for credential-shaped values — ConfigMaps are stored as plaintext in etcd, so any credential in one is a gap regardless of how it was found.

**NetworkPolicy segmentation gaps.** If the orchestrator's prompt includes K8s security scan findings, treat `check-networkpolicy-selectors.sh`'s output as the deterministic source of truth for NetworkPolicy selector issues (namespaceSelector/ipBlock MetalLB hairpin risk, missing DNS egress rules). Assess whether the flagged issue represents a real gap in the plan's context (e.g., a NetworkPolicy the plan proposes adding or modifying) versus pre-existing infrastructure debt outside this document's scope.
