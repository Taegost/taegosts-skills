...
3. Resolve default base branch (same logic as the standalone path in `SKILL.md`). Compute `BASE=$(git merge-base <base-ref> <branch-ref>)` and `git diff -U10 $BASE <branch-ref>`.
