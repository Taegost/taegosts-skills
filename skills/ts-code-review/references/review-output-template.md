...
- **Reviewer column** shows which agent(s) flagged the issue. Multiple reviewers = cross-reviewer agreement.
...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `/tmp/taegosts-skills/ts-code-review/<run-id>/`.
