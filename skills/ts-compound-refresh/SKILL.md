...
Run `ts-compound` after solving problems to start building your knowledge base.
```
...
- **Update territory** — file paths moved, classes renamed, links broke, metadata drifted, but the core recommended approach is still how the code works. `ts-compound-refresh` fixes these directly.
- **Replace territory** — the recommended solution conflicts with current code, the architectural approach changed, or the pattern is no longer the preferred way. This means a new learning needs to be written. A replacement subagent writes the successor following `ts-compound`'s document format (frontmatter, problem, root cause, solution, prevention), using the investigation evidence already gathered. The orchestrator does not rewrite learnings inline — it delegates to a subagent for context isolation.
...
After reviewing the underlying learning docs, investigate any relevant pattern docs. This repo's category mapping (`references/yaml-schema.md`) splits generalized guidance across `docs/solutions/architecture-patterns/` (agent/skill/pipeline/workflow shape decisions), `docs/solutions/design-patterns/` (reusable non-architectural design approaches), and `docs/solutions/conventions/` (team-agreed practices) — there is no single `docs/solutions/patterns/` directory. Treat docs in any of these three categories as pattern docs for this phase.
...
- **Insufficient evidence** — the drift is so fundamental that you cannot confidently document the current approach. The entire subsystem was replaced, or the new architecture is too complex to understand from a file scan alone. → Mark as stale in place:
   ...
   - Recommend the user run `ts-compound` after their next encounter with that area, when they have fresh problem-solving context
...
## Relationship to ts-compound

- `ts-compound` captures a newly solved, verified problem
- `ts-compound-refresh` maintains older learnings as the codebase evolves — both their individual accuracy and their collective design as a document set

Use **Replace** only when the refresh process has enough real evidence to write a trustworthy successor. When evidence is insufficient, mark as stale and recommend `ts-compound` for when the user next encounters that problem area.

Use **Consolidate** proactively when the document set has grown organically and redundancy has crept in. Every `ts-compound` invocation adds a new doc — over time, multiple docs may cover the same problem from slightly different angles. Periodic consolidation keeps the document set lean and authoritative.
