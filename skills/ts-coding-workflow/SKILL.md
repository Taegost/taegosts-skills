...
2. **Run `/ts-plan`** to create a plan document in `docs/plans/`
3. **Present the plan to Mike** for review. Do NOT proceed until he approves.
4. **Run `/ts-doc-review`** on the plan. This may take multiple iterations:
...
- **Never use raw `/ts-work`.** Always use `ts-do-work-loop`.
